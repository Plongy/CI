...
def github_webhook():
    ...
    try:
        ...
        os.makedirs(CLONE_FOLDER + repo_data['full_name'], exist_ok=True)
        clone_repo(repo_data['ssh_url'], f"{CLONE_FOLDER}{repo_data['full_name']}/{head_sha}")

        config = read_configfile(f"{CLONE_FOLDER}{repo_data['full_name']}/{head_sha}/{CONF}")
        ...
        os.chdir(main_dir)
        shutil.rmtree(f"{CLONE_FOLDER}{repo_data['full_name']}/{head_sha}", ignore_errors=True)

        successful_commands = [is_successful_command(*pair) for pair in zip(command_results, config['success_strings'])]
